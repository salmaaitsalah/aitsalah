...
ReactiveJammer::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ReactiveJammer")
    ...
    .AddAttribute ("ReactiveJammerReactionStrategy",
                   ...
                   UintegerValue (1), // default to fixed probability strategy
...
bool
ReactiveJammer::DoStartRxHandler (Ptr<WifiPpdu> ppdu, double startRss)
{
  NS_LOG_FUNCTION (this << ppdu << startRss);
  NS_LOG_DEBUG ("ReactiveJammer:At Node #" << GetId () <<
                ", Started receiving a packet!");

  if (IsPacketToBeJammed (ppdu->GetPsdu()))
    {
      NS_LOG_DEBUG ("ReactiveJammer:At Node #" << GetId () <<
...
bool
ReactiveJammer::IsPacketToBeJammed (Ptr<const WifiPsdu> psdu) const
{
...
ReactiveJammer::ReactToPacket (void)
{
  ...
  std::cout<< "ReactiveJammer::ReactToPacket debug 1"<< std::endl;
  if (actualPower != 0.0)
    {
      NS_LOG_DEBUG ("ReactiveJammer:At Node #" << GetId () <<
