...
double
InterferenceHelper::CurrentNodeRss (WifiTxVector txVector)
{
  double rss = CalculateNoiseFloor (txVector);
  Time now = Simulator::Now ();
  for (NiChanges::const_iterator i = m_niChanges.begin (); i != GetNextPosition(now); i++)
    {
