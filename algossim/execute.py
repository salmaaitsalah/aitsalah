...
def execute(name_dataset=str(sys.argv[1]), name_algorithm=str(sys.argv[2]), horizon=int(sys.argv[3])):
    ...
    for i in range(m1.model.horizon):
        View.displayRound(i)
        data = run(m1, m1.model, nb_instances, reward, acc, i, trial, rwd, store_data, expected, predicted, div)
        acc = data[1]
        reward = data[3]
        rwd = data[4]
        div = data[5]
...
def run(m, algo, nb_instances, reward, acc, i, trial, rwd, store_data, expected, predicted, div):
    ...
    if len(sys.argv) < 5 or len(sys.argv) > 6:
        print(str(
            sys.argv) + "4 arguments are needed: name of the dataset (mushrooms, control, or statlog), name of the "
                        ...
                        "The optional parameters are : number of node (default=9)")
    else:
...
def main():
    ...
    for i in range(1, nb_node):
        sub_folder="node{}".format(i)
        p = Process(target=execute, args=(str(sys.argv[1]), sub_folder))
        p.start()
